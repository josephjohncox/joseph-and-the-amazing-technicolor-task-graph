...
`coat-goal-store` now supports `COAT_GOAL_STORE_BACKEND=postgres` with `COAT_GOAL_STORE_DATABASE_URL` or `DATABASE_URL`. In Postgres mode the service verifies that `coat.goals` exists, writes snapshot projections transactionally, stores exact record JSONB payloads for goals, tasks, events, approvals, and artifacts, and reads the operator query endpoints back from Postgres.
...
- Add dashboard queries over the goal-store read model.
