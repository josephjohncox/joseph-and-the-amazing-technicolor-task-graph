...
<coat_control_contract>
  ...
  <security>
    ...
    <rule>You MUST ask the user before `coat_goal_submit`, `coat_approve_goal`, high-risk `coat_steer_goal`, or remote runner registration.</rule>
  </security>
</coat_control_contract>
...
<mcp_workflow>
  ...
  <step order="2" tool="coat_overview">MUST inspect service health, runners, human queues, events, and current activity before steering live work.</step>
  ...
  <step order="7" tool="coat_goal_submit">MUST only submit after explicit user confirmation.</step>
  <step order="8" tool="coat_goal_snapshot|coat_agent_activity|coat_checkpoint_history|coat_runner_list">Use for progress, prompts, evidence, checkpoints, and runner status.</step>
  <step order="9" tool="coat_approval_queue|coat_approve_goal">MUST ask before approving or rejecting on the user's behalf.</step>
  <step order="10" tool="coat_steer_goal">MUST use for user-approved pause, resume, research, retry, branch, review, or constraint directives.</step>
